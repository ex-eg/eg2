export function createQrModal({ $, t, esc, toast, uploadIcon }) {
  const qrImgSrc = (url, size = 440) =>
    'https://api.qrserver.com/v1/create-qr-code/?margin=12&size=' + size + 'x' + size + '&data=' + encodeURIComponent(url);
  ...
  function openQR(url, label) {
    ...
    $('#qrDl').onclick = async () => {
      ...
      try {
        const response = await fetch(qrImgSrc(url, 600));
        ...
        window.open(qrImgSrc(url, 600), '_blank', 'noopener');
      }
    };
  }
